...
const StatCard = ({ icon, label, value, color, delay }) => (
  <motion.div
    ...
    transition={{ delay }}
    className={`bg-mkcc-card border border-mkcc-border rounded-xl p-6 ${borderMap[color] || ''} transition-colors`}
  >
    <div className="text-3xl mb-3">{icon}</div>
    <div className="font-display text-4xl text-white">{value}</div>
    <div className="font-heading text-mkcc-muted text-sm uppercase tracking-wider mt-1">
      {label}
    </div>
  </motion.div>
);
...
export default function AdminDashboard() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-5xl text-white uppercase tracking-wide">
          Dashboard
        </h1>
        <p className="text-mkcc-muted font-body mt-1">
      ...
      >
        <h2 className="font-heading font-bold text-white text-xl mb-4 uppercase tracking-wider">
          Quick Actions
        </h2>
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            { to: '/admin/gallery',   label: '+ Add Photo',        cls: 'btn-outline text-sm py-2 px-4' },
            {
              ...
              label: '+ Announcement',
              cls: 'bg-mkcc-card border border-mkcc-border text-white font-heading text-sm py-2 px-4 rounded uppercase tracking-widest hover:border-mkcc-gold hover:text-mkcc-gold transition-colors'
            },
          ].map(({ to, label, cls }) => (
      ...
      >
        <h3 className="font-heading font-bold text-mkcc-red mb-2">
          🏏 MKCC Admin Panel
        </h3>

        <p className="text-gray-400 font-body text-sm">
          Use the sidebar to manage events, gallery, team members, membership
          requests, and announcements. All changes are reflected live on the
          public website.
        </p>
      </motion.div>
    </div>
